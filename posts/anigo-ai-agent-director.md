...
The director model was trained on a dataset of 50,000 (scene, goal, action_sequence) triples. We generated the data by recording human playthroughs of a sandbox environment and extracting the animation commands at each step:
...
The director runs at 10 Hz — fast enough for responsive behavior, slow enough that the 20ms inference time doesn't dominate the frame budget. Directives are queued and executed by the timeline system:
...
The director handles about 80% of character behavior correctly. The remaining 20% — unusual geometry, multi-step interactions — produces plausible but incorrect directives. We handle this with a validation layer that checks directives against physics and collision constraints before sending them to the timeline. If a directive fails validation, the director is queried again with the failure context appended. This retry loop rarely needs more than one retry.

The AI director replaces about 2000 lines of behavior tree code. It's less predictable than a hand-crafted tree, but far more flexible — changing character behavior is now a prompt edit rather than a tree recompile.